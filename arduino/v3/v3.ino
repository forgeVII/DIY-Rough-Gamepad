...
void loop() {
    ...
    gamepadReport.x           = 255 - (analogRead(analogPins[0]) >> 4);
    gamepadReport.y           = 255 - (analogRead(analogPins[1]) >> 4);
    gamepadReport.z           = analogRead(analogPins[3]) >> 4;
    gamepadReport.rx          = analogRead(analogPins[2]) >> 4;
    gamepadReport.ry          = analogRead(analogPins[4]) >> 4;
    gamepadReport.rz          = analogRead(analogPins[5]) >> 4;
    gamepadReport.sliderLeft  = constrain((int)((-roll + 90.0) * 255.0 / 180.0), 0, 255);
    gamepadReport.sliderRight = constrain((int)((-pitch + 90.0) * 255.0 / 180.0), 0, 255);
